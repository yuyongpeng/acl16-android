...
#define so_version "1.0"

static Acl16 acl16;

//test new way to read sn
  ...
  (JNIEnv *env, jobject jobj)
{
    ...
    if(board_select() == 3){
        char * test_str = "usid";
        fd = open("/sys/class/unifykeys/name", O_RDWR);
        ret = write(fd, test_str, 20);
        close(fd);

        fp = fopen("sys/class/unifykeys/read","r");
        fgets(sn, sizeof(sn), fp);
        fclose(fp);
    }
    else{
        ret = __system_property_get("sys.serialno", sn);
    }

    return charToJstring(env,sn);
}

JNIEXPORT jstring JNICALL Java_android_1xkkj_1api_HwAcl_so_1version
  (JNIEnv *env, jobject jobj)
{
    char version[10] = so_version;
